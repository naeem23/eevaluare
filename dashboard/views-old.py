...
def test(request):
	...
	try:
		...
		camara = None
	try:
		cover_photo = models.Photo.objects.filter(ref_no__id=4, refer_to="cover").order_by('image_order')
		summary_photo = models.Photo.objects.filter(ref_no__id=4, refer_to="summary").order_by('image_order')
	except:
		cover_photo = None
		summary_photo = None
	try:
